"""Cached daily OHLC charts for open paper positions."""
...
def render_position_chart(trade: dict, frame: pd.DataFrame) -> bytes:
    ...
    levels = [
        ("Entry", float(trade["entry_price"]), "#ffcd57", "-"),
    ...
    ax.set_title(f"{trade['ticker']} | PAPER POSITION | Daily candles", color="white", loc="left", pad=18)
    as_of = pd.Timestamp(frame.index[-1]).date().isoformat()
    fig.text(
        .08, .025,
        f"Chart through {as_of} | Yahoo Finance (may be delayed)\n"
        "Triangle = simulated entry. TP/SL levels are paper-tracking rules, not forecasts.",
